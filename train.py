...
from utils.loss_utils import l1_loss, ssim, DepthAnythingv2Loss, MapRegularizationLoss
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    depth_loss = DepthAnythingv2Loss(iter_from=opt.depth_from, iter_end=opt.depth_end, end_mult=opt.depth_end_mult,
                                     source_path=dataset.source_path, cameras=viewpoint_stack)
    map_loss = MapRegularizationLoss()
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # if depth_loss.is_active(iteration):
        #     loss_depth = opt.lambda_depthanythingv2 * depth_loss(viewpoint_cam, render_pkg, iteration)
        #     loss += loss_depth
        #     # print(opt.lambda_depthanythingv2 * depth_loss(viewpoint_cam, render_pkg, iteration))
        # loss_map = map_loss(render_pkg['raw_L'], gt_image)
        # loss += loss_map
        loss.backward()
        ...
        with torch.no_grad():
            ...
            mse = torch.nn.functional.mse_loss(render_pkg['clear_render'], viewpoint_cam.high_original_image.cuda())
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    gaussians.densify_and_prune(opt.densify_grad_threshold, 0.005, scene.cameras_extent, size_threshold,
                                                radii)
                    print("현재 Gaussian 수: ", gaussians.get_xyz.shape[0])
