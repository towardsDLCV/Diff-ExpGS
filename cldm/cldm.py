...
class ControlledUnetModel(UNetModel):
    def forward(self, x, timesteps=None, context=None, control=None, only_mid_control=False,
        ...
        with torch.no_grad():
            ...
            h = x.type(x.dtype)
            if ref_zt is not None:
                ref_zt = ref_zt.type(x.dtype)
            for module in self.input_blocks:
                h = module(h, emb, context)
                if ref_zt is not None:
                    ref_zt = module(ref_zt, emb, context)
                hs.append(h)
            h = self.middle_block(h, emb, context, ref=ref_zt)
...
class ControlLDM(LatentDiffusion):
    ...
    def __init__(self, control_stage_config, control_key, only_mid_control, *args, **kwargs):
        ...
        with open("./resources/empty_embedding.pkl", 'rb') as f:
    ...
    def _interp_alpha_sigma(self, t_cont):
        """
        t_cont: (B,) continuous in [0,1], from DPM-Solver wrapper
        returns alpha_t (B,1,1,1), sigma_t (B,1,1,1)
        """
        # self.alphas_cumprod: (N,)
        device = t_cont.device
        N = self.alphas_cumprod.shape[0]
        t_array = torch.linspace(0., 1., N + 1, device=device)[1:]  # (N,)
        log_alpha_array = 0.5 * torch.log(self.alphas_cumprod.to(device))  # (N,)

        # clamp to [t_1, t_N]
        t = t_cont.clamp(min=t_array[0].item(), max=t_array[-1].item())  # (B,)

        idx_high = torch.searchsorted(t_array, t, right=True).clamp(max=N - 1)  # (B,)
        idx_low = (idx_high - 1).clamp(min=0)
        t_low = t_array[idx_low]
        t_high = t_array[idx_high]
        w = ((t - t_low) / (t_high - t_low + 1e-8)).unsqueeze(-1)  # (B,1)

        la_low = log_alpha_array[idx_low]
        la_high = log_alpha_array[idx_high]
        log_alpha_t = (1 - w) * la_low.unsqueeze(-1) + w * la_high.unsqueeze(-1)  # (B,1)

        alpha_t = torch.exp(log_alpha_t).view(-1, 1, 1, 1)  # (B,1,1,1)
        sigma_t = torch.sqrt(1. - torch.exp(2. * log_alpha_t)).view(-1, 1, 1, 1)  # (B,1,1,1)
        return alpha_t, sigma_t

    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        assert isinstance(cond, dict)
        ref = kwargs.get('ref', None)
        ref_z0 = kwargs.get('ref_z0', None)
        ...
        ref_zt = None
        if ref_z0 is not None:
            ref_z0 = ref_z0.to(device=x_noisy.device, dtype=x_noisy.dtype)
            if t.dtype in (torch.float32, torch.float64):
                alpha_t, _ = self._interp_alpha_sigma(t)
            else:
                ac = self.alphas_cumprod.to(x_noisy.device)
                alpha_t = torch.sqrt(ac[t.long()]).view(-1, 1, 1, 1)
            ref_zt = alpha_t * ref_z0
    ...
    def change_first_stage(self, checkpoint_file):
        ...
        state_dict = torch.load(checkpoint_file, map_location=torch.device("cpu"), weights_only=False)["state_dict"]
        new_state_dict = {}
        for s in state_dict:
            if "my_vae" in s:
